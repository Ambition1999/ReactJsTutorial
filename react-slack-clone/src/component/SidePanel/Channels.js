...
class Channels extends React.Component {
    ...
    setActivateChannel = channel => {
        this.setState({activateChannel: channel.id});
    }

    displayChannels = channels => (
        channels.length > 0 && channels.map(channel => (
            <Menu.Item key={channel.id} onClick = {() => this.changeChannel(channel)} name ={channel.name} style ={{opacity: 0.7}} active={channel.id === this.state.activateChannel}>
                # {channel.name}
            </Menu.Item>
        ))
    )
